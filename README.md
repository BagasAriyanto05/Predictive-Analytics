...
- **Pembangunan Model**. Pada pembangunan model terdapat beberapa algorima yang digunakan, antara lain :
  ...
  - **Random Forest**
    <br> Random Forest (RF) adalah suatu algoritma yang digunakan pada klasifikasi data dalam jumlah yang besar. Klasifikasi random forest dilakukan melalui penggabungan pohon (tree) dengan melakukan training pada sampel data yang dimiliki. Penggunaan pohon (tree) yang semakin banyak akan mempengaruhi akurasi yang akan didapatkan menjadi lebih baik. Penentuan klasifikasi dengan random forest diambil berdasarkan hasil voting dari tree yang terbentuk. Pemenang dari tree yang terbentuk ditentukan dengan vote terbanyak. Proses klasifikasi pada random forest berawal dari memecah data sampel yang ada kedalam decision tree secara acak. Setelah pohon terbentuk,maka akan dilakukan voting pada setiap kelas dari data sampel. Kemudian, mengkombinasikan vote dari setiap kelas kemudian diambil vote yang paling banyak.Dengan menggunakan random forest pada klasifikasi data maka, akan menghasilkan vote yang paling baik [[4](https://id.wikipedia.org/wiki/Random_forest)]. Kelebihan dan kekurangan algoritma Random Forest adalah sebagai berikut (bersumber dari [[5](https://medium.com/swlh/random-forest-classification-and-its-implementation-d5d840dbead0)]) :
...
- **Informasi Dataset**
  <br> Dataset yang digunakan pada proyek ini yaitu crop dataset, informasi lebih lanjut  mengenai dataset tersebut dapat lihat pada tabel 1 :
...
Berdasarkan hasil visualisasi data pada gambar 1 sampai gambar 7, dapat terlihat sebaran atau distribusi data yang ada pada setiap fitur. Termasuk nilai minimum, median, maksimum, Q1, Q3, batas atas dan batas bawah. Selain itu dapat dilihat juga pada beberapa fitur masih terdapat nilai outliers.
  
 Visualisasi data yang menunjukkan sebaran/distribusi data pada fitur target (`label`) yang ditunjukan oleh gambar 8:
...
Berdasarkan hasil visualisasi dari fitur target 'label' pada gambar 8 dapat memberikan informasi bahwa dataset sudah seimbang dengan jumlah sampel masing-masing label yaitu 100 sampel, sehingga tidak perlu menyeimbangkan data lagi.
...
Gambar 15. Perbandingan N, P, L terhadap label crop
 ...
 Hasil visualisasi pada gambar 9 sampai gambar 15 memberikan informasi mengenai rata-rata kandungan N, P, K terhadap setiap label crop. Dimana dapat dilihat bahwa terdapat beberapa label crop yang membutuhkan lahan dengan kandungan N,P,K tinggi dan beberapa label membutuhkan lahan dengan kandungan N,P,K rendah.
  
  Visualisasi rata-rata tingkat `temperature, humidity dan rainfall` terhadap setiap label ditunjukan oleh gambar 16 sampai gambar 22 :
...
- **Korelasi antar Fitur Numerik**
  <br> _Correlation Matrix_ antar fitur numerik ditunjukan pada gambar 23 :
  
  <img width="467" alt="Submission 1 MLT - Korelasi antar fitur numerik" src="https://user-images.githubusercontent.com/92978028/199405885-bc2e6fa9-2790-4ab5-8031-5172863461db.png">
...
|weighted avg|0\.9865981240981241|0\.9863636363636363|0\.9863742019264262|440\.0|
  ...
  Berdasarkan hasil pengujian yang ditunjukan oleh tabel 2 dapat dilihat bahwa model dengan algoritma K-Nearest Neighbor memperoleh nilai akurasi yaitu sebesar 0.986364.
...
Berdasarkan hasil pengujian yang ditunjukan oleh tabel 4 dapat dilihat bahwa model dengan algoritma Random Forest memperoleh nilai akurasi yaitu sebesar 0.997727.
...
Tabel 5. Perbandingan akurasi, presisi, dan recall dari ketiga algoritma
...
Dari tabel 5 dapat memberikan informasi bahwa ketiga model yang dibangun memiliki nilai akurasi di atas 98%. Dimana dapat dilihat juga bahwa model dengan algoritma Random Forest memiliki performa (nilai akurasi, precision, dan recall) yang lebih baik dari model dengan algoritma K-Nearest Neighbor dan XGBoost. 
...
Dari hasil penelitian diatas dapat diambil kesimpulan bahwa dari dataset yang dipakai dalam pembuatan model dapat digunakan dengan baik di ketiga algoritma yang dipakai pada proyek penelitian kali ini. Dan dari hasil perbandingan ketiga algoritma yang dipakai, yaitu algoritma K-Nearest Neighbor, algoritma XRBoost, dan algoritma Random Forest sama-sama mendapatkan hasil diatas 98%, namun dari ketiga algoritma yang digunakan nilai tertinggi accuracy, precission, dan recall didapatkan oleh algoritma Random Forest dengan rata-rata nilai 99,79%.
